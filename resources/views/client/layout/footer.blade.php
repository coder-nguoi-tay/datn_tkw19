        <footer class="footer  mt-50 pt-30 " style="--jobbox-footer-bg-image: url()">
            <div class="container">
                <div class="row justify-content-start">
                    <div class="footer-col-1 col-md-3 col-sm-12">
                        <div id="block-7" class="widget widget_block widget_media_image">
                            <h3>ITWORK</h3>
                        </div>
                        <div id="block-13" class="widget widget_block widget_text">
                            <p class="small mt-20 mb-20">ITOWORK là trang tuyển dụng it nhanh chóng và 
                                tiện lợi. Với mong muốn giúp mọi người có thể tìm kiếm công việc phù hợp với bản thân
                            </p>
                        </div>

                    </div>
                    <div class="footer-col-2 col-md-2 col-xs-6">
                        <div id="nav_menu-2" class="widget widget_nav_menu">
                            <h6 class="mb-20">Công ty</h6>
                            <nav class="menu-resources-container" aria-label="Resources">
                                <ul id="menu-resources" class="menu">
                                    <li id="menu-item-155"
                                        class="menu-item menu-item-type-post_type menu-item-object-page menu-item-155">
                                        <a href="../about-us/index.html">Về chúng tôi</a>
                                    </li>
                                    <li id="menu-item-153"
                                        class="menu-item menu-item-type-post_type menu-item-object-page menu-item-153">
                                        <a href="../candidates/index.html">Tuyển dụng</a>
                                    </li>
                                    <li id="menu-item-154"
                                        class="menu-item menu-item-type-post_type menu-item-object-page menu-item-154">
                                        <a href="../contact-us/index.html">Liên hệ</a>
                                    </li>
                                </ul>
                            </nav>
                        </div>
                    </div>
                    <div class="footer-col-3 col-md-2 col-xs-6">
                        <div id="nav_menu-3" class="widget widget_nav_menu">
                            <h6 class="mb-20">Cộng đồng ITWORK</h6>
                            <nav class="menu-community-container" aria-label="Community">
                                <ul id="menu-community" class="menu">
                                    <li id="menu-item-156"
                                        class="menu-item menu-item-type-custom menu-item-object-custom menu-item-156"><a
                                            href="#">Facebook Fanpage</a></li>
                                    <li id="menu-item-157"
                                        class="menu-item menu-item-type-custom menu-item-object-custom menu-item-157"><a
                                            href="#">Youtube Official Channel</a></li>
                                    <li id="menu-item-158"
                                        class="menu-item menu-item-type-custom menu-item-object-custom menu-item-158"><a
                                            href="#">Linkedin</a></li>
                                    <li id="menu-item-159"
                                        class="menu-item menu-item-type-custom menu-item-object-custom menu-item-159"><a
                                            href="#">Tiktok Official Channel</a></li>
                                </ul>
                            </nav>
                        </div>
                    </div>
                    <div class="footer-col-4 col-md-2 col-xs-6">
                        <div id="nav_menu-4" class="widget widget_nav_menu">
                            <h6 class="mb-20">Hồ sơ và CV</h6>
                            <nav class="menu-quick-links-container" aria-label="Quick links">
                                <ul id="menu-quick-links" class="menu">
                                    <li id="menu-item-149"
                                        class="menu-item menu-item-type-custom menu-item-object-custom menu-item-149">
                                        <a href="#">Quản lý CV của bạn</a>
                                    </li>
                                    <li id="menu-item-150"
                                        class="menu-item menu-item-type-custom menu-item-object-custom menu-item-150">
                                        <a href="#">ITWORK Profile</a>
                                    </li>
                                    <li id="menu-item-151"
                                        class="menu-item menu-item-type-custom menu-item-object-custom menu-item-151">
                                        <a href="#">Hướng dẫn viết CV</a>
                                    </li>
                                    <li id="menu-item-152"
                                        class="menu-item menu-item-type-custom menu-item-object-custom menu-item-152">
                                        <a href="#">Review CV</a>
                                    </li>
                                </ul>
                            </nav>
                        </div>
                    </div>
                    <div class="footer-col-5 col-md-2 col-xs-6">
                        <div id="nav_menu-5" class="widget widget_nav_menu">
                            <h6 class="mb-20">Điều khoản</h6>
                            <nav class="menu-legal-menu-container" aria-label="More">
                                <ul id="menu-legal-menu" class="menu">
                                    <li id="menu-item-160"
                                        class="menu-item menu-item-type-custom menu-item-object-custom menu-item-160">
                                        <a href="#">Hỏi đáp</a>
                                    </li>
                                    <li id="menu-item-161"
                                        class="menu-item menu-item-type-custom menu-item-object-custom menu-item-161">
                                        <a href="#">Chính sách bảo mật</a>
                                    </li>
                                    <li id="menu-item-162"
                                        class="menu-item menu-item-type-custom menu-item-object-custom menu-item-162">
                                        <a href="#">Điều khoản dịch vụ</a>
                                    </li>
                                    <li id="menu-item-163"
                                        class="menu-item menu-item-type-custom menu-item-object-custom menu-item-163">
                                        <a href="#">Quy chế hoạt động</a>
                                    </li>
                                </ul>
                            </nav>
                        </div>
                    </div>
                </div><!-- .row -->
            </div><!-- .container -->
            <div class="container">
                <div class="footer-bottom mt-50">
                    <div class="row gy-3">
                        <div class="col-md-6">
                            <span class="font-xs color-text-paragraph">Copyright &copy; 2023. ITWORK all right
                                reserved</span>
                        </div>
                        <div class="col-md-6 text-md-end text-start">
                            <div class="footer-social">
                                <a href="#" class="font-xs color-text-paragraph">Privacy Policy</a>
                                <a href="#" class="font-xs color-text-paragraph ml-30">Terms &#038;
                                    Conditions</a>
                                <a href="#" class="font-xs color-text-paragraph ml-30">Security</a>
                            </div>

                        </div>
                    </div>
                </div>
            </div>
        </footer>
