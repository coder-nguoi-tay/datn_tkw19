<template>
  <nav class="musashi-pagination" v-if="pager.pages && pager.pages.length">
    <!-- <span class="total-item-pagination">{{ (pager.currentPage - 1) * pager.pageSize + 1 }} -
      {{
          pager.pageSize * pager.currentPage > pager.totalItems
            ? pager.totalItems
            : pager.pageSize * pager.currentPage
      }}
      / 全 {{ pager.totalItems }}件</span> -->
    <ul v-if="pager.pages && pager.pages.length" class="pagination" :style="ulStyles">
      <li class="page-item first" v-if="pager.currentPage != 1" :class="{ disabled: pager.currentPage === 1 }"
        :style="liStyles">
        <a class="page-link" @click="setPage(1)" :style="aStyles">{{
            labels.first
        }}</a>
      </li>
      <li class="page-item previous" v-if="pager.currentPage != 1" :class="{ disabled: pager.currentPage === 1 }"
        :style="liStyles">
      ...
      <li class="page-item next" v-if="pager.currentPage !== pager.totalPages"
      ...
      <li class="page-item last" v-if="pager.currentPage !== pager.totalPages"
        :class="{ disabled: pager.currentPage === pager.totalPages }" :style="liStyles">
