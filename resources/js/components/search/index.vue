      ...
      <div class="container header__container">
        <div class="header__wrapper">
          <form action="" class="form-inline">
            <input name="heaeder_search" class="form-control header-search" placeholder="&#xF002;&ensp;検索" value=""
            ...
            <div class="header-filter" @click="showSearchSidebar">
              <img class="header-filter-icon" src="/assets/img/user/common/icon_filter.png" />
            </div>
          </form>
        </div>
      </div>
        ...
        <div class="event-list__wrapper pt-0" v-if="data.events.length">
          <div class="cat-list ongoing-list reserved-list">
            <div class="event-item__container">
              <div class="event-item__wrapper" v-for="item in data.events.filter((x) => x.waiting_publish)"
                :key="item.id">
                    ...
                    <div class="event-right__container">
                      ...
                      <div class="event-meta d-flex align-items-center flex-wrap">
                        <span class="meta-item">
                          <img src="/assets/img/user/event/ic_period_2.svg" /><span>残り...</span><b>{{ item.remaining_day
                          }}日と<span class="txt-time">{{
                            item.remaining_hour
                            }}</span>時間<span class="txt-time">{{
                              item.remaining_minute
                              }}</span>分</b>
                        </span>
                        <span class="meta-item">
                          <img src="/assets/img/user/event/ic_people_2.svg" /><b><span class="txt-number">{{
                      ...
                      <div class="event-tag">
                        <a v-for="tag in item.event_tags.filter((x) => x.tag)" :key="tag.id" href=""># {{ tag.tag.name
                        }}</a>
                      </div>
                    </div>
        ...
        <div class="event-nodata__wrapper h-100" v-else>
          <div class="event-nodata">
            ...
            <div class="btn-nodata">
              <a :href="data.urlCreate" class="btn-create-event">イベントを企画する</a>
            </div>
          </div>
        </div>
        <div v-if="data.events.length" class="btn-nodata d-btn-create-event">
          <a :href="data.urlCreate" class="btn-create-event">イベントを新しく作成する</a>
        </div>
      </div>
    </div>
